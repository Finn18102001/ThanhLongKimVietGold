...
const { isServerlessHost } = require("../lib/runtime-env");
...
module.exports = function apiRouter(_ROOT) {
   ...
   * Trên Vercel/serverless: client dùng Supabase Realtime trực tiếp (không giữ kết nối SSE).
   */
  router.get("/gold-table/stream", function (req, res) {
    ...
    if (!url || !key) {
      console.warn(
        "[TLKV gold-push] GET /api/gold-table/stream → thiếu SUPABASE env — client fallback Realtime"
      );
      res.status(204).end();
      return;
    }
