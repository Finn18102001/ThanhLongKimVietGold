  ...
  async function onSubmit(formData: FormData) {
    ...
    try {
      ...
      setAlert({
        ...
        reason: `Đã nhận ${received} chiếc ${sku?.name ?? ""}. Tồn kho đã cộng và ghi sổ PURCHASE_RECEIVED.`,
      ...
      setAlert({
        ...
        detail: "Kho chưa đổi. Không có phiếu nhập và không có dòng sổ cái.",
      });
    } finally {
      setPending(false);
    }
  ...
  return (
    <section className="rounded-[12px] bg-white p-5 shadow-[var(--tlkv-shadow)]">
      ...
      <p className="mt-1 text-[12px] text-[var(--tlkv-muted)]">
        Chỉ cộng kho khi xác nhận nhận thực tế. Mỗi lần thành công tạo phiếu nhập + sổ cái.
      </p>
