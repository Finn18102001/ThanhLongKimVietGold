-- DEPOSIT (in-stock hold / partial / full-but-undelivered) uses invoice prefix DH
-- (đơn đặt hàng). Only SALE with gold delivered keeps HD.
...
begin
  ...
  begin
    ...
    if v_tx_type = 'SALE' and (v_remaining > 0 or v_pickup is not null) then
      ...
      -- Full/partial/0 pay but gold held → đơn đặt hàng (DH), not HD
      v_invoice_no := 'DH' || lpad(nextval('public.pos_invoice_seq')::text, 6, '0');
