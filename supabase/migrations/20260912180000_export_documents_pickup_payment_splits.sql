-- Additive CSV fields for Hóa đơn export (no logic change to sale/stock):
-- pickupDueAt, goldDeliveredAt, firstPaidDong (đặt cọc/lần 1), secondPaidDong (lần 2 / phần còn lại đã thu).
-- goldDeliveredAt = earliest PREORDER_FULFILL or SALE inventory tx for that sale (when stock actually left).
