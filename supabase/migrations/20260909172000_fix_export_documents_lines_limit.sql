-- Restore line-level CSV export (product / qty / weight) and allow limiting to N
-- most recent documents. Prior supplier migration overwrote export as list-only.

drop function if exists public.pos_export_documents(text, text, date, date, text);
...
declare
  ...
  -- 0 / null => all matching docs (hard cap 5000 docs). Else 1..2000.
