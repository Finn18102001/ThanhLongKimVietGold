-- Additive: expose sale transaction_type + fulfillment_status on pos_list_documents
-- so Hóa đơn list can show "Chưa trả vàng" instead of treating every sale as DELIVERED.
-- Buy/receipt rows stay SALE + DELIVERED (goods already received at those document types).
-- JSON keys are additive; existing clients ignore unknown fields.
