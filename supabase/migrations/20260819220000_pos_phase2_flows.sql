-- Phase 2: Stock Count, Return, Category, Audit list, Reporting snapshot

create sequence if not exists public.pos_stock_count_seq;
create sequence if not exists public.pos_return_seq;
...
create table if not exists public.pos_stock_counts (
  ...
  constraint pos_stock_counts_scope_check check (scope_type in ('ALL', 'CATEGORY')),
  constraint pos_stock_counts_status_check check (
    status in ('DRAFT', 'COUNTING', 'PENDING_APPROVAL', 'APPROVED', 'COMPLETED', 'REJECTED')
  )
);

create table if not exists public.pos_stock_count_items (
  id uuid primary key default gen_random_uuid(),
  count_id uuid not null references public.pos_stock_counts(id) on delete restrict,
  sku_id uuid not null references public.pos_skus(id) on delete restrict,
  system_qty integer not null,
  actual_qty integer,
  constraint pos_stock_count_items_unique unique (count_id, sku_id),
  constraint pos_stock_count_items_system_nonneg check (system_qty >= 0),
  constraint pos_stock_count_items_actual_nonneg check (actual_qty is null or actual_qty >= 0)
);

create index if not exists pos_stock_counts_created_idx
  on public.pos_stock_counts (created_at desc);
create index if not exists pos_stock_count_items_count_idx
...
create table if not exists public.pos_returns (
  ...
  actor_email text not null,
  created_at timestamptz not null default now(),
  constraint pos_returns_status_check check (status in ('COMPLETED')),
  constraint pos_returns_refund_check check (refund_method in ('CASH', 'TRANSFER', 'CARD')),
  constraint pos_returns_total_nonneg check (total_dong >= 0),
  constraint pos_returns_reason_check check (length(trim(reason)) > 0)
);
...
create table if not exists public.pos_return_items (
  ...
  constraint pos_return_items_qty_positive check (quantity > 0),
  constraint pos_return_items_money_nonneg check (
    unit_price_dong >= 0 and total_price_dong >= 0
  )
);

create index if not exists pos_returns_created_idx
  on public.pos_returns (created_at desc);
create index if not exists pos_return_items_sale_item_idx
  on public.pos_return_items (sale_item_id);

-- ---------------------------------------------------------------------------
-- POS categories (browse groups — does not replace website product CRUD)
-- ---------------------------------------------------------------------------
...
create table if not exists public.pos_categories (
  ...
  constraint pos_categories_status_check check (status in ('ACTIVE', 'INACTIVE')),
  constraint pos_categories_name_check check (length(trim(name)) > 0)
);

create table if not exists public.pos_category_skus (
  category_id uuid not null references public.pos_categories(id) on delete cascade,
  sku_id uuid not null references public.pos_skus(id) on delete cascade,
  primary key (category_id, sku_id)
);

create index if not exists pos_categories_order_idx
  on public.pos_categories (display_order, name);
...
alter table public.pos_category_skus enable row level security;

create policy pos_stock_counts_admin_select on public.pos_stock_counts
  for select to authenticated using (public.tlkv_is_admin());
create policy pos_stock_count_items_admin_select on public.pos_stock_count_items
  for select to authenticated using (public.tlkv_is_admin());
create policy pos_returns_admin_select on public.pos_returns
  for select to authenticated using (public.tlkv_is_admin());
create policy pos_return_items_admin_select on public.pos_return_items
  for select to authenticated using (public.tlkv_is_admin());
create policy pos_categories_admin_select on public.pos_categories
  for select to authenticated using (public.tlkv_is_admin());
create policy pos_category_skus_admin_select on public.pos_category_skus
  for select to authenticated using (public.tlkv_is_admin());

revoke all on public.pos_stock_counts from anon, authenticated;
revoke all on public.pos_stock_count_items from anon, authenticated;
revoke all on public.pos_returns from anon, authenticated;
revoke all on public.pos_return_items from anon, authenticated;
revoke all on public.pos_categories from anon, authenticated;
revoke all on public.pos_category_skus from anon, authenticated;

grant select on public.pos_stock_counts to authenticated;
grant select on public.pos_stock_count_items to authenticated;
...
grant select on public.pos_category_skus to authenticated;
